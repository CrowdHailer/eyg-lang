...
// TODO acc = sub, nex, types
// step(acc, env, source, ref, t0, e0, k)
    ...
    Ok(exp) ->
      case exp {
        ...
        e.Fn(x, e1) -> {
          ...
          let #(sub, next, types) =
            unify_at(type_, t.Fun(arg, new_eff, ret), sub, next, types, ref)
        ...
        e.Builtin(identifier) ->
          fetch(builtins(), identifier, sub, next, types, ref, type_, k)
        literal -> {
...
fn primitive(exp, next) {
  case exp {
    e.Var(_) | e.Call(_, _) | e.Fn(_, _) | e.Let(_, _, _) | e.Builtin(_) ->
      panic("not a literal")
