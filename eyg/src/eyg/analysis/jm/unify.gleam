...
fn do_unify(constraints, s, next)  {
  ...
  case constraints {
    [] -> Ok(#(s, next))
    [#(t1, t2), ..rest] -> case t.apply(s, t1), t.apply(s,t2) {
      t.Var(i), t.Var(j) if i == j -> do_unify(rest, s, next)
      t.Var(i), t1 | t1, t.Var(i) -> case set.contains(t.ftv(t1), i) {
        True -> Error(error.RecursiveType)
        False -> do_unify(rest, map.insert(s, i, t1), next)
      }
    t.Fun(a1, e1, r1), t.Fun(a2, e2, r2) -> do_unify([#(a1, a2), #(e1, e2), #(r1, r2), ..rest], s, next)
    t.Integer, t.Integer -> do_unify(rest, s, next)
    t.String, t.String -> do_unify(rest, s, next)
    t.LinkedList(i1), t.LinkedList(i2) -> do_unify([#(i1, i2), ..rest], s, next)
    ...
    t.RowExtend(label1, value1, tail1), t.RowExtend(_,_,_) as row2 -> {
        use #(value2, tail2, s1, next) <- result.then(rewrite_row(label1, row2, s, next))
        // case tail1 {
        //   t.Var(x) -> {
    ...
    t.EffectExtend(label1, #(lift1, reply1), tail1), t.EffectExtend(_,_,_) as row2 -> {
        ...
        do_unify([#(lift1, lift2), #(reply1, reply2), #(tail1, tail2), ..rest], s, next)
      }
