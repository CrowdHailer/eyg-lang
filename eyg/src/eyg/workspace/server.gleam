    ...
    #(
      ...
      r.BuiltinFn(fn(object) {
        ...
        case int.parse(content) {
          Ok(value) -> Ok(r.Tagged("Ok", native(value)))
...
fn std_string() {
  r.Record([#("split", r.BuiltinFn(fn(object) { todo("string split") }))])
}
...
pub fn boot(source) {
  ...
  let processes = []
  assert Ok(cont) =
    stepwise.step(source, prelude(), fn(value) { Ok(stepwise.Done(value)) })
  ...
  case server {
    r.Function(arg, body, env, _) ->
      Ok(fn(method, path, body) {
        let request =
          r.Record([
        ...
        assert Ok(cont) =
          stepwise.step_call(
            server,
            request,
            fn(value) { Ok(stepwise.Done(value)) },
          )
        assert Ok(#(processes, messages, response)) =
          stepwise.loop(cont, processes, [])
        let processes = deliver_messages(processes, messages)
        real_js.set_reference(ref, processes)
        // Need to handle variable type
        case response {
          r.Binary(content) -> content
          r.Function(pattern, body, captured, _self) -> // TODO HTML heading
            ...
            // Pass out the pids
            list.map(
              map.to_list(captured)
              |> list.append([#("doo", response)]),
              r.render_var,
            )
