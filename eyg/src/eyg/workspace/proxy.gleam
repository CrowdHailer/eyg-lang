        ...
        ["", "123456", ..path] -> {
          ...
          case effectful.eval_call(term, request, effectful.real_log) {
            Ok(r.Binary(content)) -> {
              let reply =
                encode.object([
                  #("status", encode.integer(200)),
                  #("body", encode.string(content)),
                ])
              let url = string.join([api, "response", response_id], "/")
              post_json(url, reply)
              Ok(state)
            ...
            Error(reason) -> {
              let reply =
                encode.object([
                  #("status", encode.integer(500)),
                  #("body", encode.string(reason)),
                ])
              let url = string.join([api, "response", response_id], "/")
              post_json(url, reply)
              Ok(state)
...
const response_type = t.Record(
  [#("status", t.Native("Integer", [])), #("body", t.Binary)],
  None,
)
  ...
  t.Function(
    ...
    t.Union(
      [
        ...
        #(
          "HTTP",
          t.Function(t.Union([#("Get", t.Binary)], None), t.Binary, t.empty),
        ),
      ],
      None,
    ),
...
pub type State =
  Result(#(Dynamic, e.Node(Dynamic, Dynamic)), String)
