...
pub fn run_browser(source) {
  let r = try_catch(fn() { start(source, map.new()) })
}
...
pub fn fetch(o, ref) {
  ...
  promise.map(
    ...
    fn(body) {
      ...
      assert Ok(cont) =
        step_call(callback, r.Binary(body), fn(value) { Ok(Done(value)) })
      assert Ok(#(processes, messages, value)) = loop(cont, processes, [])
      let messages = list.reverse(messages)
      let #(processes, key_handler) =
        deliver_messages(
          processes,
          messages,
          r.Binary("we ignore this key handler"),
        )
...
pub fn start(source, env) {
  ...
  let handler =
    r.Function(
      ...
      e.call(
        e.variable("send"),
        e.tuple_([e.access(e.variable("system"), "ui"), e.variable("key")]),
      ),
      env,
      None,
    )
...
pub fn handle_keydown(state, key) {
  ...
  let processes = real_js.dereference(ref)
  assert Ok(cont) =
    step_call(key_handler, r.Binary(key), fn(value) { Ok(Done(value)) })
  assert Ok(#(processes, messages, _value)) = loop(cont, processes, [])
  let messages = list.reverse(messages)
  // send -> dispatch ~~~> deliver -> receive
  // TODO this should be r.function
  let #(processes, key_handler) =
    deliver_messages(processes, messages, key_handler)
  real_js.set_reference(ref, processes)
  #(ref, key_handler)
}
...
fn deliver_messages(processes, messages, key_handler) {
  case messages {
    ...
    [r.Tuple([r.Pid(i), message]), ..rest] -> {
      ...
      assert Ok(cont) =
        step_call(process, message, fn(value) { Ok(Done(value)) })
      ...
      case effect {
        Spawn(func) -> {
          ...
          let value =
            r.Function(
              p.Variable("then"),
              e.call(e.variable("then"), e.variable("pid")),
              map.new()
              |> map.insert("pid", r.Pid(pid)),
              None,
            )
        ...
        Send(message) -> {
          ...
          let value =
            r.Function(
              p.Variable("then"),
              e.call(e.variable("then"), e.tuple_([])),
              map.new(),
              None,
            )
          assert Ok(value) = cont(value)
...
pub fn step_call(func, arg, cont: fn(r.Object) -> Result(Cont, String)) {
  ...
  case func {
    ...
    r.BuiltinFn(func) if func == se -> Ok(Eff(Send(arg), cont))
    r.BuiltinFn(func) -> {
      try value = func(arg)
      Ok(Cont(value, cont))
    }
