...
pub fn rasing_effect_test() {
  ...
  let assert r.Effect("Foo", lifted, k) = r.eval(source, env.empty(), r.done)
  lifted
...
pub fn effect_in_case_test() {
  ...
  let assert r.Effect("Raise", lifted, _k) = r.eval(source, env.empty(), r.done)
  lifted
...
pub fn effect_in_builtin_test() {
  ...
  let assert r.Effect("Foo", lifted, k) = r.eval(source, stdlib.env(), r.done)
  lifted
...
pub fn multiple_effects_test() {
  ...
  let assert r.Effect("Choose", lifted, k) = r.eval(source, env.empty(), r.done)
  lifted
  |> should.equal(r.Record([]))
