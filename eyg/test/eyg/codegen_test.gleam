...
pub fn variable_assignment_test() {
  ...
  let js = compile(untyped, #(typer.init(browser.native_to_parameters), typer.root_scope([])))
  let [l1, l2, l3] = js
  let "let foo$1 = \"V1\";" = l1
  let "let foo$2 = foo$1;" = l2
  let "foo$2" = l3
  assert True =
    dynamic.from("V1") == eval(untyped, #(typer.init(browser.native_to_parameters), typer.root_scope([])))
}

pub fn nested_assignment_test() {
  let state = #(typer.init(browser.native_to_parameters), typer.root_scope([]))
  let untyped =
    ast.let_(
  ...
  let "match$1" = l5
  assert True =
    dynamic.from("TMP!") == eval(untyped, #(typer.init(browser.native_to_parameters), typer.root_scope([])))
}
...
pub fn tuple_term_test() {
  ...
  assert True =
    dynamic.from(#("abc", "xyz")) == eval(
      untyped,
      #(typer.init(browser.native_to_parameters), typer.root_scope([])),
    )
}
...
pub fn tuple_destructure_test() {
  ...
  let "a$1" = l2
  assert True =
    dynamic.from("x") == eval(untyped, #(typer.init(browser.native_to_parameters), typer.root_scope([])))
}
...
pub fn record_assignment_test() {
  ...
  assert True =
    ...
    ])) == eval(untyped, #(typer.init(browser.native_to_parameters), typer.root_scope([])))
}
...
pub fn multiline_record_assignment_test() {
  ...
  let js = compile(untyped, #(typer.init(browser.native_to_parameters), scope))
  let [l1, l2, l3, l4, l5, l6, l7] = js
  let "{" = l1
  let "  first_name: (() => {" = l2
  let "    let tmp$1 = \"TMP!\";" = l3
...
pub fn record_destructure_test() {
  ...
  let js =
    compile(
      untyped,
      #(
        typer.init(browser.native_to_parameters),
        typer.root_scope([
...
pub fn multiline_function_test() {
  ...
  let state = #(typer.init(browser.native_to_parameters), scope)
  let js = compile(untyped, state)
  let [l1, l2, l3, l4] = js
