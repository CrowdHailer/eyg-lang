...
pub fn tuples_test() {
  ...
  assert Ok(r.Tuple([r.Tuple([]), r.Binary("hello")])) =
    tree_walk.eval(source, empty)
  assert Ok(r.Tuple([r.Tuple([]), r.Binary("hello")])) =
    tail_call.eval(source, empty)
  // I wonder if handling Ok/Error makes interpreter slower
  assert Ok(r.Tuple([r.Tuple([]), r.Binary("hello")])) =
    stepwise.eval(source, empty)
}
...
pub fn tuple_patterns_test() {
  ...
  assert Ok(r.Tuple([r.Binary("bar"), r.Binary("foo")])) =
    tree_walk.eval(source, empty)
  assert Ok(r.Tuple([r.Binary("bar"), r.Binary("foo")])) =
    tail_call.eval(source, empty)
  assert Ok(r.Tuple([r.Binary("bar"), r.Binary("foo")])) =
...
pub fn incorrect_tuple_size_match_test() {
  let empty = map.new()
  let source = e.let_(p.Tuple(["x", "y"]), e.tuple_([]), e.tuple_([]))
...
pub fn not_a_tuple_match_test() {
  let empty = map.new()
  let source = e.let_(p.Tuple(["x", "y"]), e.binary(""), e.binary(""))
...
pub fn functions_test() {
  let empty = map.new()
  let source =
    e.call(e.function(p.Variable("x"), e.variable("x")), e.tuple_([]))
...
pub fn invalid_match_test() {
  let empty = map.new()
  let source =
    e.case_(e.binary("not a union"), [#("False", p.Tuple([]), e.binary("no"))])
  assert Error(_) = tree_walk.eval(source, empty)
  assert Error(_) = tail_call.eval(source, empty)
  assert Error(_) = stepwise.eval(source, empty)
}
...
pub fn recursive_test() {
          ...
          [
            ...
            #(
              ...
              e.let_(
                ...
                e.call(
                  e.variable("move"),
                  e.tuple_([e.variable("from"), e.variable("to")]),
                ),
              ),
            ),
          ],
  ...
  let empty =
    ...
    |> map.insert(
      "x",
      r.Tuple([cons(r.Binary("1"), cons(r.Binary("2"), tail())), tail()]),
    )
  assert Ok(r.Tagged(
    "Cons",
    r.Tuple([
      r.Binary("2"),
      r.Tagged("Cons", r.Tuple([r.Binary("1"), r.Tagged("Nil", r.Tuple([]))])),
    ]),
  )) = tree_walk.eval(source, empty)
  assert Ok(r.Tagged(
    "Cons",
...
pub fn builtin_test() {
  ...
  let source =
    e.call(e.access(e.variable("string"), "reverse"), e.binary("hello"))
