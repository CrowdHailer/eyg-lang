...
pub fn type_bound_function_test() {
  ...
  assert t.Function(t.Tuple([]), t.Binary, _) = resolve(type_, substitutions)
}
...
pub fn call_function_test() {
  ...
  let #(typed, typer) = infer(untyped, t.Tuple([]), t.empty, #(typer, scope))
  let typer.Typer(substitutions: substitutions, ..) = typer
  assert Ok(type_) = get_type(typed)
...
pub fn call_with_incorrect_argument_test() {
  ...
  let #(typed, _state) = infer(untyped, t.Tuple([]), t.empty, #(typer, scope))
...
pub fn generic_reuse_function_test() {
  ...
  let #(typed, typer) =
    infer(untyped, t.Tuple([t.Tuple([]), t.Binary]), t.empty, #(typer, scope))
  let typer.Typer(substitutions: substitutions, ..) = typer
  assert Ok(type_) = get_type(typed)
