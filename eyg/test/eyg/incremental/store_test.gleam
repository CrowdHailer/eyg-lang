...
pub fn fn_poly_test() {
  ...
  let tree = e.Let("id", e.Lambda("x", e.Variable("x")), e.Apply(e.Variable("id"), e.Integer(10)))
  let #(root, s) = store.load(s, tree)
  should.equal(root, 5)
  should.equal(map.size(s.source), 6)
...
pub fn nested_fn_test()  {
  ...
  let assert Ok(#(t, s)) = store.type_(s, root)
  should.equal(t, t.Fun(t.Unbound(0), t.Closed, t.Fun(t.Unbound(1), t.Closed, t.Record(t.Closed))))
  should.equal(map.size(s.free), 3)
  should.equal(map.size(s.types), 3)
  ...
  let assert Ok(#(t, s)) = store.type_(s, root1)
  should.equal(t, t.Fun(t.Unbound(2), t.Closed, t.Fun(t.Unbound(3), t.Closed, t.Integer)))
  let assert Ok(#(t, s)) = store.type_(s, root2)
  should.equal(t, t.Fun(t.Unbound(4), t.Closed, t.Fun(t.Unbound(5), t.Closed, t.Unbound(5))))
...
pub fn branched_apply_test() {
  ...
  let tree = e.Let("id", e.Lambda("x", e.Variable("x")),
  e.Apply(e.Apply(e.Variable("id"), e.Variable("id")), e.Apply(e.Variable("id"), e.Integer(1))))
