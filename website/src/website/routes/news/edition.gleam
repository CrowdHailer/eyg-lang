...
fn main(sections) {
  h.div([a.styles([#("padding", "1.5rem 1.5rem 0rem 1.5rem")])], sections)
}

pub fn header(issue_url, number, pea_src) {
  h.div(
    [a.styles([#("padding", ".5rem 1.5rem"), #("background", mint_green)])],
    [
      h.table(
        [
...
fn footer(edition_url) {
  h.div(
    [a.styles([#("background", mint_green), #("padding", ".5rem 1.5rem")])],
    [
      block(
...
fn p(children) {
  h.p(
    [a.styles([#("margin-top", "20px"), #("margin-bottom", "20px")])],
    children,
  )
}
