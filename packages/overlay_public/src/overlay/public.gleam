...
pub fn main() -> Nil {
  ...
  let config = case ollama_api_key() {
    "" -> None
    api_key ->
      case is_development() {
        True -> {
          let assert Ok(proxy_origin) = origin.from_string(browser_origin())
          Some(ollama.Config(origin: proxy_origin, api_key: Some(api_key)))
        }
        False -> Some(ollama.cloud(api_key))
      }
  }
