...
/// The entity currently authorised to publish releases under `package`, if any.
...
/// Append an ownership record to both grant and transfer ownership;
/// `current_package_owners` always reflects the latest record.
pub fn record_owner(
  package: String,
  entity_id: String,
) -> pog.Query(PackageOwner) {
