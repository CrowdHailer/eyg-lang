...
pub type Action {
  ...
  Share(origin: origin.Origin, source: ir.Node(Nil))
}
...
pub fn share(client, source) {
  let Client(origin:, ..) = client
  #(client, [Share(origin:, source:)])
}

pub type Message {
  ReleasesFetched(Result(schema.PullResponse, ledger_client.Failure))
  FragmentFetched(
    cid: v1.Cid,
    result: Result(Response(BitArray), fetch.FetchError),
  )
  ModuleShared(Result(Response(BitArray), fetch.FetchError))
}
...
pub fn update(client: Client, message) -> #(Client, _) {
  case message {
    ReleasesFetched(result) -> {
      case result {
        Ok(schema.PullResponse(entries:)) -> {
    ...
    FragmentFetched(cid:, result:) -> {
      case result {
        ...
        Ok(Response(status:, ..)) -> {
          echo "unexpected status: " <> int.to_string(status)
          #(client, [])
        }
        Error(reason) -> {
          echo #("network error", reason)
...
fn do_run(task, dispatch: fn(Message) -> Nil) {
  case task {
    SyncFrom(origin:, since:) -> {
      ...
      promise.map(fetchx.send_bits(request), fn(response) {
        let result = case response {
          Ok(response) -> client.pull_packages_response(response)
          Error(reason) ->
            Error(ledger_client.NetworkError(string.inspect(reason)))
        }
        [dispatch(ReleasesFetched(result))]
      })
    }
    ...
    Share(origin:, source:) -> {
      ...
      let request = operation.to_request(operation, origin)
      promise.map(fetchx.send_bits(request), fn(response) {
        [dispatch(ModuleShared(response))]
      })
    }
  }
}
