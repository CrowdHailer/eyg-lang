...
pub fn update(state: State, message) {
  case message {
    ...
    InputMessage(message) -> {
      ...
      case mode, message {
        doc.Manipulating(id, manipulation.EnterInteger(value, rebuild)), _ ->
          case input.update_number(value, message) {
            input.Continue(new) -> {
              let mode =
                doc.Manipulating(id, manipulation.EnterInteger(new, rebuild))
        ...
        doc.Manipulating(id, manipulation.EnterText(value, rebuild)), _ ->
          case input.update_text(value, message) {
            input.Continue(new) -> {
              let mode =
                doc.Manipulating(id, manipulation.EnterText(new, rebuild))
              let state = State(..state, mode:)
              #(state, [])
            }
            input.Confirmed(value) -> {
    ...
    PickerMessage(message) -> {
      let State(mode:, ..) = state
      case mode {
        doc.Manipulating(id, manipulation.PickSingle(_picker, rebuild)) ->
          case message {
            picker.Updated(picker:) -> {
              let mode =
                doc.Manipulating(id, manipulation.PickSingle(picker, rebuild))
              #(State(..state, mode:), [])
            }
        ...
        doc.Manipulating(id, manipulation.PickCid(_picker, rebuild)) ->
          case message {
            picker.Updated(picker:) -> {
              let mode =
                doc.Manipulating(id, manipulation.PickCid(picker, rebuild))
              #(State(..state, mode:), [])
            }
            picker.Decided(text) -> {
    ...
    ClipboardReadCompleted(return) -> {
      case state.mode {
        doc.ReadingFromClipboard(id, rebuild) ->
          case return {
            Ok(text) ->
              case json.parse(text, dag_json.decoder(Nil)) {
                Ok(expression) -> {
                  ...
                  let state = State(..state, mode: doc.Navigating(id, None))
                  #(state, [])
                }
    ...
    ModuleLookupCompleted(cid, result) -> {
      ...
      case state.mode {
        doc.Running(id, run.Fetching(module:, env:, k:)) ->
...
fn user_pressed_key(state, key) {
  ...
  case mode, key {
    _, "Escape" -> #(State(..state, mode: doc.UnFocused), [])
    ...
    doc.Navigating(id, _error), _ -> {
      let mode = doc.Navigating(id, Some(doc.NoKeyBinding(key)))
      #(State(..state, mode:), [])
    }
    doc.UnFocused, _ -> #(state, [])
    doc.ReadingFromClipboard(id: _, rebuild: _), _ -> #(state, [])
    doc.Running(id: _, status: _), _ -> #(state, [])
    doc.Manipulating(id: _, input: _), _ -> #(state, [])
  }
}
...
fn action_failed(state, id, name) {
  let state =
    State(
      ..state,
      mode: doc.Navigating(id:, failure: Some(doc.ActionFailed(name))),
    )
  #(state, [])
}
