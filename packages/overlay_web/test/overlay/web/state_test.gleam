...
pub fn init_with_ollama_local_test() {
  let #(state, actions) = state.init(None)
  let assert [_] = actions
  assert provider.Ollama(ollama.local()) == state.llm.provider
}

pub fn init_with_ollama_cloud_test() {
  let api_key = "ollama-api-key"
  let config = ollama.cloud(api_key)
  let #(state, actions) = state.init(Some(config))
  let assert [_] = actions
  assert provider.Ollama(config) == state.llm.provider
}
...
pub fn submit_prompt_test() {
  ...
  let assert [browser.FetchStreamResponse(request, resume)] = actions
  assert Some(11_434) == request.port
  ...
  let assert browser.Done(message) =
    resume(
      response.new(200)
  ...
  let assert [browser.ReadChunk(_reader, resume)] = actions

  let message = resume(Ok(Some(helpers.ollama_chunk_encode("Hi there"))))

  let #(state, actions) = state.update(state, message)
  let assert state.Streaming(_r, _) = state.status
  let assert [browser.ReadChunk(_, resume)] = actions
  let message = resume(Ok(None))
...
pub fn interrupted_stream_test() {
  let #(state, actions) = submit_first_prompt("hello")
  let assert [browser.FetchStreamResponse(_request, resume)] = actions
  let response =
    Ok(response.new(200) |> response.set_body(new_reader([], Ok(Nil))))
  let #(state, actions) = state.update(state, resume(response))
  let assert [browser.ReadChunk(_reader, resume)] = actions
  let message = resume(Error(fetch.NetworkError("Testing")))
  let #(state, actions) = state.update(state, message)
  let assert Some(_) = state.input_error
  assert [] == actions
}
...
pub fn eval_response_from_llm_test() {
  ...
  let state = State(..state.new(None), status:)
  ...
  let assert [browser.FetchStreamResponse(request:, resume: _)] = actions
  assert Some(11_434) == request.port
...
pub fn side_effect_test() {
  ...
  // assert run_code(id, code) == call
  // let assert run.Handling(..) = run
  // assert [] == pending
  // let assert [browser.Alert("Hello World", resume:)] = actions
  // let message = resume()
  // let #(state, actions) = state.update(state, message)
...
pub fn module_remains_in_context_for_second_tool_call_test() {
  ...
  let #(state, actions) = state.update(state, state.LlmStreamFinished(Ok(Nil)))
  // let assert state.Executing(_, run.Pending(dep:, ..), []) = state.status
  // assert cache.Content(cid) == dep
  // assert [] == actions

  // let #(state, actions) =
  //   state.update(state, state.ModuleLookupCompleted(cid, Ok(ir.integer(43))))
  ...
  //   == state.status
  // let assert [browser.FetchStreamResponse(..)] = actions
}
...
pub fn module_lookup_failure_remains_executing_test() {
  let assert Ok(#(cid, _)) =
    v1.from_string(
      "bafyreigdmqpykrgxyahdnfmfzmc5j4bkwci6wf6fkdbapq7hfpmg2j3yqy",
    )
  ...
  let #(state, actions) =
    state.update(state, state.ModuleLookupCompleted(cid, Error("not fetched")))
  // let assert state.Executing(call, run.Pending(dep:, ..), pending) =
  //   state.status
  // assert run_code(id, code) == call
  // assert cache.Content(cid) == dep
  // assert [] == pending
...
pub fn invalid_module_is_reported_to_llm_test() {
  ...
  let #(state, actions) =
    state.update(state, state.ModuleLookupCompleted(cid, Ok(ir.vacant())))
